...
- Name: Message signing >> BIP-322 message signing >> Signature approved
- Location: /home/runner/work/extension/extension/tests/specs/rpc-message-signing/bip322-message-signing.spec.ts:33:5
...
-     "message": "test",
-     "signature": "AkcwRAIgBt6EMsCmR5Wy3NL1mDhviAddDFFSVK7EMNV4Z6fOfsECIGsXBBT2x8sMtBJFvjQwuf5BFMQqbFqGZzQRWy0/V2IhASEDA0e+UAqLJwegDnV2wMUnokfN3G6DY+5RFHuOQ7WQuqk=",
+   "error": Object {
+     "code": 4001,
...
+   "jsonrpc": "2.0",
    ...
    at /home/runner/work/extension/extension/tests/specs/rpc-message-signing/bip322-message-signing.spec.ts:42:27
```
...
```ts
  ...
  11 |     });
  12 |
  13 |     function clickActionButton(context: BrowserContext) {
  14 |       return async (buttonToPress: 'Cancel' | 'Sign') => {
  ...
  26 |               message,
  27 |               paymentType: 'p2wpkh',
  28 |             }).catch((e: unknown) => e),
  29 |           message
  30 |         );
  31 |     }
  32 |
  33 |     test('Signature approved', async ({ page, context }) => {
  34 |       const [result] = await Promise.all([
  35 |         initiateMessageSigning(page)('test'),
  36 |         clickActionButton(context)('Sign'),
  37 |       ]);
  38 |
  39 |       // ID is random, removed so we can test known values
  40 |       delete result.id;
  41 |
> 42 |       test.expect(result).toEqual({
     |                           ^ Error: expect(received).toEqual(expected) // deep equality
  43 |         jsonrpc: '2.0',
  44 |         result: {
  45 |           signature:
  46 |             'AkcwRAIgBt6EMsCmR5Wy3NL1mDhviAddDFFSVK7EMNV4Z6fOfsECIGsXBBT2x8sMtBJFvjQwuf5BFMQqbFqGZzQRWy0/V2IhASEDA0e+UAqLJwegDnV2wMUnokfN3G6DY+5RFHuOQ7WQuqk=',
  47 |           address: 'bc1q530dz4h80kwlzywlhx2qn0k6vdtftd93c499yq',
  48 |           message: 'test',
  49 |         },
  50 |       });
  51 |     });
  52 |
  53 |     test('Signature rejected', async ({ page, context }) => {
  54 |       const [result] = await Promise.all([
  55 |         initiateMessageSigning(page)('test'),
  56 |         clickActionButton(context)('Cancel'),
  57 |       ]);
  58 |
  59 |       delete result.id;
  60 |
  61 |       test.expect(result).toEqual({
  62 |         jsonrpc: '2.0',
  63 |         error: {
  64 |           code: 4001,
  65 |           message: 'User rejected request',
  66 |         },
  67 |       });
  68 |     });
  69 |   }));
