...
index 7ffa5956d..297c6a985 100644
...
@@ -21,21 +21,19 @@ import { defineRpcRequestHandler } from '../rpc-message-handler';
 import {
   RequestParams,
   createConnectingAppSearchParamsWithLastKnownAccount,
-  getTabIdFromPort,
...
+    sendResponse(
       createRpcErrorResponse(method, {
         id: request.id,
         error: { code: RpcErrorCode.INVALID_REQUEST, message: 'Parameters undefined' },
@@ -46,8 +44,7 @@ async function handleRpcSignStacksMessage(
...
+    sendResponse(
       createRpcErrorResponse(method, {
         id: request.id,
         error: {
@@ -61,17 +58,17 @@ async function handleRpcSignStacksMessage(
 
   void trackRpcRequestSuccess({ endpoint: method });
 
-  const { urlParams, tabId } = await createConnectingAppSearchParamsWithLastKnownAccount(
-    port,
+  const { urlParams } = await createConnectingAppSearchParamsWithLastKnownAccount(
+    sender,
     requestParams
   );
...
-  sendErrorResponseOnUserPopupClose({ tabId, id, request });
+  await triggerRequestPopupWindowOpen(RouteUrls.RpcStacksSignature, urlParams);
+  // sendErrorResponseOnUserPopupClose({ tabId, id, request });
 }
...
@@ -89,13 +86,13 @@ export const stxSignMessageHandler = defineRpcRequestHandler(
       ]);
     }
...
+  async (request, sender, sendResponse) => {
     const requestParams: RequestParams = [
       ['requestId', request.id],
       ['messageType', 'structured'],
@@ -113,6 +110,6 @@ export const stxSignStructuredMessageHandler = defineRpcRequestHandler(
       ],
     ];
...
index 31436c254..e2a500580 100644
...
@@ -102,12 +119,12 @@ export function listenForOriginTabClose({ tabId }: ListenForOriginTabCloseArgs)
...
@@ -115,12 +132,12 @@ export function createConnectingAppMetadataSearchParams(
 }
...
@@ -146,20 +163,21 @@ interface ValidateRequestParamsArgs {
   id: string;
   method: RpcMethodNames;
   params: unknown;
-  port: chrome.runtime.Port;
...
@@ -171,8 +189,7 @@ export function validateRequestParams({
...
+    sendResponse(
       createRpcErrorResponse(method, {
         id,
         error: {
@@ -185,3 +202,23 @@ export function validateRequestParams({
   }
